/* social.js - Updated with Shop Badges */
...
window.viewFriendProfile = function(targetName) {
    ...
    firebase.database().ref('users/' + targetName).once('value').then((snapshot) => {
        ...
        if (inventory.length > 0) {
            badgesHtml = `<div class="flex flex-wrap gap-2 justify-center mb-4 pt-2 border-t border-slate-100 dark:border-slate-700">`;
            if(inventory.includes('badge_crown')) badgesHtml += `<div class="p-1.5 bg-amber-100 text-amber-600 rounded" title="Golden Crown"><i data-lucide="crown" class="w-4 h-4"></i></div>`;
            if(inventory.includes('badge_star')) badgesHtml += `<div class="p-1.5 bg-purple-100 text-purple-600 rounded" title="Super Star"><i data-lucide="star" class="w-4 h-4"></i></div>`;
            if(inventory.includes('badge_fire')) badgesHtml += `<div class="p-1.5 bg-orange-100 text-orange-600 rounded" title="Hot Streak"><i data-lucide="flame" class="w-4 h-4"></i></div>`;
            if(inventory.includes('badge_zap')) badgesHtml += `<div class="p-1.5 bg-indigo-100 text-indigo-600 rounded" title="Efficient"><i data-lucide="zap" class="w-4 h-4"></i></div>`;
            if(inventory.includes('theme_emerald')) badgesHtml += `<div class="p-1.5 bg-emerald-100 text-emerald-600 rounded" title="Supporter"><i data-lucide="leaf" class="w-4 h-4"></i></div>`;
            badgesHtml += `</div>`;
        }
...
function renderLeaderboard(filter = '') {
    ...
    firebase.database().ref('users').once('value').then((snapshot) => {
        ...
        users.forEach((user, index) => {
            ...
            // --- RENDER SMALL BADGES NEXT TO NAME ---
            const inventory = user.inventory || [];
            let rowBadges = '';
            // Only show max 3 badges in list view to keep it clean
            let badgeCount = 0;
            if(inventory.includes('badge_crown')) { rowBadges += '<i data-lucide="crown" class="w-3 h-3 text-amber-500 fill-current"></i>'; badgeCount++; }
            if(inventory.includes('badge_star') && badgeCount < 3) { rowBadges += '<i data-lucide="star" class="w-3 h-3 text-purple-500 fill-current"></i>'; badgeCount++; }
            if(inventory.includes('badge_fire') && badgeCount < 3) { rowBadges += '<i data-lucide="flame" class="w-3 h-3 text-orange-500 fill-current"></i>'; badgeCount++; }
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="flex items-center gap-1.5">
                            <p class="text-sm font-bold text-slate-700 dark:text-slate-200">${user.name}</p>
                            ${rowBadges}
